...
Verify your file system and storage virtual machine (SVM1) has been created.
...
Use `helm install` to install Trident driver in the trident namespace.
[source,bash]
----
...
// [source,shell]
// ----
// cd ~/rosa-fsx/fsx/trident-installer/helm && helm install trident -n trident trident-operator-22.10.0.tgz
// ----
...
Use the the `fsx/svm_secret.yml` file

`cd` to the fsx directory
[source,bash]
----
cd ~/rosa-fsx/fsx
----
...
Next, we'll assign the ManagementIP to the SVMIP variable.
...
Now, create the storage class.  You can use the `cat` command to view any files before applying them.
...
Note: `password` is the default password but can be chagned in the `mysql-secret.yml` file
...
Next we will deploy MySQL StatefulSet application on the ROSA cluster. Open `mysql/mysql-statefulset.yml` and review the details –metadata, replicas, and storageclass name. Then, execute the following command.  For simplicity in this lab, we are only going to create one (1) replica set.
...
Make sure you are in the fsx directory
[srouce,bash]
----
cd ~/rosa-fsx/fsx
----
...
[source, bash]
----
oc get volumesnapshots -n mysql 
----
...
[source,bash]
----
oc apply -f restore-snapshot-to-pvc.yml
----

Then create a new MySQL application using the new pvc.

[source,bash]
----
oc apply -f mysql/mysql-deployment-clone.yml
----
...
[source,bash]
----
oc exec -n mysql -it mysql-fsx-7db4f45b8-tpdb4 -- mysql -u root -p
----
...
Type `quit` and then you have completed the lab!