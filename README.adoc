...
Next we will deploy a MySQL application on the ROSA cluster. Open `mysql/mysql-deployment.yml` and review the details –metadata, replicas, and storageclass name. Then, execute the following command.  For simplicity in this lab, we are only going to create one (1) replica set.
