...
// aws cloudformation describe-stacks --stack-name ROSA-FSXONTAP
...
Use the the `svm_secret.yml` file in the `fsx` directory
...
// NOTE: `ManagementLIF` and `DataLIF` IP Addresses can be found via the Amazon FSx Console under `Storage virtual machines` -> `Endpoints` as shown in the following screenshot:

// image::images/smv1.png[]

Use this command to view the details for the SVM
...
mysql-set-1   1/1     Running   0          22s
mysql-set-2   1/1     Running   0          8s
----
